...
import { vaccinations as vaccApi } from "@/lib/api";
...
export default function EditVaccinationPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    vaccApi.getById(Number(vaccinationId)).then((data: Record<string, unknown>) => {
      setVaccineName((data.vaccineName as string) || "");
      setStatusType((data.status as string) === "completed" ? "completed" : "pending");
      setScheduledDate((data.scheduledDate as string) || "");
      setAdministeredDate((data.administeredDate as string) || new Date().toISOString().split("T")[0]);
      setDosage((data.dosage as string) || "");
      setRoute((data.route as string) || "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data: Record<string, unknown> = {
        ...
        scheduledDate: statusType === "completed" ? (administeredDate || scheduledDate) : scheduledDate,
        status: statusType,
      ...
      if (statusType === "completed") {
        ...
        data.administeredDate = null;
      }
      await vaccApi.update(Number(vaccinationId), data as never);
      navigate("/farm/vaccinations");
      toast.success("Vaccination updated successfully");
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Failed to update vaccination");
      setSaving(false);
    }
  }

  if (loading) {
    return <LoadingSpinner text="Loading vaccination..." />;
  }
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Vaccination</h1>
          <p className="text-sm text-gray-500 mt-0.5">Update vaccination details</p>
        </div>
      </div>
