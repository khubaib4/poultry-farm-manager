          ...
          <tbody>
            {data.weeklyData.map((w, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-right border border-gray-200">{Number(w.eggsTotal ?? 0).toLocaleString()}</td>
                <td className="px-3 py-2 text-right border border-gray-200">{w.deaths > 0 ? <span className="text-red-600">{w.deaths}</span> : "0"}</td>
                <td className="px-3 py-2 text-right border border-gray-200">{Number(w.feedKg ?? 0).toFixed(1)}</td>
                <td className="px-3 py-2 text-right border border-gray-200">Rs {Number(w.expenses ?? 0).toLocaleString()}</td>
              </tr>
            ))}
            <tr className="bg-gray-100 font-semibold">
              <td className="px-3 py-2 border border-gray-200">Monthly Total</td>
              <td className="px-3 py-2 text-right border border-gray-200">{Number(data.totals.eggsTotal ?? 0).toLocaleString()}</td>
              <td className="px-3 py-2 text-right border border-gray-200">{data.totals.deaths}</td>
              <td className="px-3 py-2 text-right border border-gray-200">{Number(data.totals.feedKg ?? 0).toFixed(1)}</td>
              <td className="px-3 py-2 text-right border border-gray-200">Rs {Number(data.financial.expenses ?? 0).toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="border border-gray-200 rounded-lg p-4">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span>Total Revenue</span><span className="font-semibold text-green-600">Rs {Number(data.financial.revenue ?? 0).toLocaleString()}</span></div>
            <div className="flex justify-between"><span>Total Expenses</span><span className="font-semibold text-red-600">Rs {Number(data.financial.expenses ?? 0).toLocaleString()}</span></div>
              ...
              <div className="space-y-1">
                {[...data.financial.expensesByCategory].sort((a, b) => b.amount - a.amount).map(ec => (
                  <div key={ec.category} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">Rs {Number(ec.amount ?? 0).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
